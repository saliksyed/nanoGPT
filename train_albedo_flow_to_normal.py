...
class EncoderBlock(nn.Module):
    ...
    def __init__(
        ...
        self.mlp_flow = MLPBlock(hidden_dim, mlp_dim, dropout)

    def forward(self, input: torch.Tensor):
        torch._assert(
            input.dim() == 4,
            f"Expected (batch_size, seq_length, hidden_dim, type_dim) got {input.shape}",
        )
        albedo_input = input[:, :, :, 0]
        flow_input = input[:, :, :, 1]
        x = self.ln_1(albedo_input)
        y = self.ln_2(flow_input)
        x_p, _ = self.self_attention(x, x, x, need_weights=False)
        x_p = self.dropout(x_p) + albedo_input
        x_pp = x_p + self.cross_attention(self.ln_3(x_p), y, y)[0]
        flow_next = flow_input + self.mlp(self.ln_5(flow_input))
        return torch.stack([x_pp + self.mlp(self.ln_4(x_pp)), flow_next], dim=-1)
...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        self.pos_embedding = nn.Parameter(
            torch.empty(1, seq_length, hidden_dim, 2).normal_(std=0.02)
        )  # from BERT
    ...
    def forward(self, input: torch.Tensor):
        torch._assert(
            input.dim() == 4,
            f"Expected (batch_size, seq_length, hidden_dim, type) got {input.shape}",
        )
        input = input + self.pos_embedding

        output = self.layers(self.dropout(input))
        o1 = self.ln_1(output[:, :, :, 0])
        o2 = self.ln_2(output[:, :, :, 1])
...
class ViTEncoder(nn.Module):
    def __init__(
        ...
        self.conv_proj1 = nn.Conv2d(
            in_channels=3,
            out_channels=hidden_dim,
            kernel_size=patch_size,
            stride=patch_size,
        )
        self.conv_proj2 = nn.Conv2d(
            in_channels=3,
            out_channels=hidden_dim,
            kernel_size=patch_size,
            stride=patch_size,
    ...
    def forward(self, input: torch.Tensor):
        ...
        n_h = h // p
        n_w = w // p
        x = self.conv_proj1(input[:, :, :, :, 0])
        y = self.conv_proj2(input[:, :, :, :, 1])
        x = x.reshape(n, self.hidden_dim, n_h * n_w)
        x = x.permute(0, 2, 1)
        y = y.reshape(n, self.hidden_dim, n_h * n_w)
        y = y.permute(0, 2, 1)
        return self.encoder(torch.stack([x, y], dim=-1))
...
class DepthPredictor(nn.Module):
    def __init__(
        ...
        self.transform_1 = nn.Linear(hidden_dim, decoder_dim)
        self.transform_2 = nn.Linear(hidden_dim, decoder_dim)
    ...
    def forward(self, input: torch.Tensor, targets: torch.Tensor = None):
        ...
        x = self.encoder(x)
        x1 = self.transform_1(x[:, :, :, 0])
        x2 = self.transform_2(x[:, :, :, 1])
        x = self.decoder(torch.stack([x1, x2], dim=-1))
        x = x[:, :, :, 0]
...
checkpoint_path = "weights/albedo_flow_to_normal.pt"
...
ax[0].imshow(viz)
ax[1].imshow(outputs[idx].permute(1, 2, 0))
plt.show()
